...
- **Edges**: the relationships between individuals in a network. Also called "ties"
...
Note: Rank is determined by Davids Score. A higher Davids score denotes a higher rank.

Now we can create the network objects and add node attributes to the objects. Note that adding node attributes to objects involves '%v%', which resembles a pipe. This functions similarly to a $ operator and allows us to add rank variables to a node.'%e%' adds attributes to edges. 
...
The plot on the left looks great, but we can make it more informative by adding edge weights and color the nodes by rank. This will enable us to have an intuitive understanding of how much individuals interact with one another based on their rankings! 
```{r visualize}
par(mfrow = c(1, 2))
...
What if we included multiple predictor variables? We can add another network as a predictor variable! By doing so, we are conducting a multiple regression QAP. For example, we can now ask: **Do sender rank and grooming relationships predict the number of times bonobos GG rubbed in the network?**
...
We need to use two networks with the same number of nodes so we chose to ask a question about grooming and contact: **Does grooming relationships correlate with contact relationships in spider monkeys?**
...
We looked at using this method in social networks, but you could also use it in any network.
Some examples include anything with nodes that are not people such as:
- Transit stops and the edges would be commuters moving between them
- Academic papers and the citations being the edges 
