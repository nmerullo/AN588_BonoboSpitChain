...
knitr::include_graphics("https://www.researchgate.net/publication/327368736/figure/fig1/AS:666036512768011@1535806887938/Representation-of-Social-Networks-using-a-a-Graph-or-b-Matrix.png")
# trying to find a pic of matrix and edge list here
```
...
There are 3 types of QAP regressions we can run with {statnet}: correlation regressions, linear regressions, and logistic regressions. We will describe and demonstrate each regression using the bonobo data. We'll also do the same with a more complex dataset from spider monkeys! 
...
summary(qap_cor)
```
Under "Estimated p-values", we can see that our p-values are very high and not significant, meaning we can accept our null hypothesis. There is no significant correlation between grooming and GG rubbing networks. Under "Test Diagnostics", the test value represents the correlation coefficient between grooming and GG rubbing networks. We previously calculated this using the gcor() function.

[Why are there 2 p-values? How to interpret distribution summary? -Sam]

We can also visualize this comparison using the plot() function.
```{r qap correlation plot}
plot(qap_cor)
```
[Describe what we see here -Sam]

## 2. Linear regression
Linear regression QAPs test the extent to which predictor variables are affecting edge weights. <br>
...
Logistic regression QAPs test the extent to which independent variables are affecting the presence or absence of edges. If you have a network with binary ties, we can use this regression method to predict ties in the outcome network (response variable) <br>
