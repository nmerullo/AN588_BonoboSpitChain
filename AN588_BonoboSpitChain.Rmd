...
```{r bonobo edge list}
bonobo_gg_edgelist <- bonobo_gg[,1:3] #only columns 2 and 3 have actors and receivers
head(bonobo_gg_edgelist)
bonobo_groom_edgelist <- bonobo_groom[,1:3] #only columns 2 and 3 have actors and receivers
...
bonobo_gg_net <- as.network.data.frame(bonobo_gg_edgelist)
bonobo_groom_net<-as.network.data.frame(bonobo_groom_edgelist)
par(mfrow = c(1, 2))
...
## Correlation
```{r}
qap_cor <- qaptest(list(bonobo_gg_net, bonobo_groom_net), # Include both network objects in a list. 
...
Independent variables must be matrices in order to be used in the QAP functions.
We can look at whether rank affects the likelihood of sending ties or receiving ties (or both, but they must be added to the model separately).
...
rank <- bonobo_gg$rank.act # a vector of the node-level variable we are interested in
rank_sending <- matrix(data = NA, nrow = nodes, ncol = nodes)
...
for (i in 1:nodes){ # for 1 through the number of nodes in the data set
rank_receiving[,i] <- rep(nodes, rank[i])} # The age of each actor is repeated over entire COLUMN of matrix
rank_receiving
```


```{r}
mrqap <- netlm(bonobo_gg_net, # dependent variable is a network object with a weighted adjacency matrix
                list(rank_sending)) # list of all independent variables as network objects or matrix objects
...
```{r}
lrqap <- netlogit(bonobo_gg_net, # dependent variable is a network object with an unweighted adjacency matrix
                   list()) # list of all independent variables as network objects or matrix objects
