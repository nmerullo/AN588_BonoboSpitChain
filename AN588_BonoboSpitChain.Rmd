...
```{r}
lrqap <- netlogit(bonobo_gg_net, # response variable is a network object with an unweighted adjacency matrix
                   list()) # list of all predictor variables as network objects or matrix objects
...
sp_mating_net <- as.network(mating_spmonkey, matrix.type = "edgelist", directed = TRUE, multiple = TRUE)
...
sp_play_net <- as.network(play_spmonkey, matrix.type = "edgelist", directed = TRUE, multiple = TRUE)
plot(sp_play_net, label = "vertex.names", main="Spidermonkey Play")

#Adding network and edge attributes to the network
sp_play_net %v% "sex" <- sp_attr$sex #adds sex as a node attribute
sp_play_net %v% "age" <- sp_attr$age #adds age as a node attribute
sp_play_net %v% "repr" <- sp_attr$Repr_status #adds reproductive status as a node attribute

#let's add the "Mins" from the initial spmonkey edgelist as edge weights
sp_play_net %e% "time" <- spmonkey_el[spmonkey_el$Beh == "play",]$Mins 
```
```{r sp_groom}
groom_spmonkey <- spmonkey_el[spmonkey_el$Beh == "groom",][,-c(2,4)] #making a separate data frame for grooming behavior
sp_groom_net <- as.network(groom_spmonkey, matrix.type = "edgelist", directed = TRUE, multiple = TRUE, loops = TRUE)
...
sp_contact_net <- as.network(contact_spmonkey, matrix.type = "edgelist", directed = TRUE, multiple = TRUE, loops = TRUE)
